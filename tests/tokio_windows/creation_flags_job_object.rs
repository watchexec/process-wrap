...
impl SpawnProvider for InspectProvider {
	fn validate_attempt(&self, attempt: &SpawnAttempt, _command: &CommandWrap) -> Result<()> {
		...
		assert_eq!(attempt.kills_on_drop(), self.0.kill_on_drop);
		...
		assert_eq!(policy.kills_on_drop(), self.0.kill_on_drop);
...
#[test]
fn portable_policy_covers_flags_jobs_suspension_and_kill_on_drop() {
	...
	flags_only
		.wrap(CreationFlags(PROCESS_CREATION_FLAGS(flags)))
		.wrap(KillOnDrop);
	assert_policy(
		flags_only,
		ExpectedPolicy {
	...
	for order in [Order::CreationFlagsFirst, Order::JobObjectFirst] {
		let mut temporary = command(PROCESS_CREATION_FLAGS(flags), order);
		temporary.wrap(KillOnDrop);
		assert_policy(
			temporary,
			ExpectedPolicy {
				user_flags: flags,
...
#[tokio::test]
async fn suspended_native_children_are_killed_after_later_failures() -> Result<()> {
	for failure in [Failure::Error, Failure::Panic] {
		for fail_before_job in [false, true] {
			...
			child.start_kill()?;
			let _ = wait_for_exit(child.as_mut()).await?;
		}
	}
	Ok(())
}
