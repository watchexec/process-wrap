...
pub(super) fn prepare_environment<'a>(
	inherits: bool,
	changes: impl IntoIterator<Item = (&'a OsStr, Option<&'a OsStr>)>,
) -> io::Result<PreparedEnvironment> {
	prepare_environment_with(inherits, changes, inherited_environment)
}

pub(super) fn prepare_environment_with<'a>(
	inherits: bool,
	changes: impl IntoIterator<Item = (&'a OsStr, Option<&'a OsStr>)>,
	...
	let changes = changes
		...
		.map(|(key, value)| {
			Ok(match value {
				Some(value) => EncodedChange::Set(EnvironmentVariable {
					key: encode_key(key)?,
					value: encode(value, "PTY environment value")?,
				}),
				None => EncodedChange::Remove(encode_key(key)?),
			})
		})
		.collect::<io::Result<Vec<_>>>()?;

	if inherits && changes.is_empty() {
...
mod tests {
	...
	fn prepare_environment_with(
		...
		super::prepare_environment_with(
			!intent.clear,
			intent.changes.iter().map(|change| match change {
				EnvChange::Set(key, value) => (key.as_os_str(), Some(value.as_os_str())),
				EnvChange::Remove(key) => (key.as_os_str(), None),
			}),
			capture,
		)
	}
