...
pub(super) fn prepare_command_line(
	program: &OsStr,
	args: &[CommandArg],
) -> io::Result<PreparedCommandLine> {
	let program = encode(program, "PTY program")?;
...
mod tests {
	...
	fn regular(value: &str) -> CommandArg {
		CommandArg::Regular(OsString::from(value))
	}

	fn raw(value: &str) -> CommandArg {
		CommandArg::Raw(OsString::from(value))
	}

	fn prepare(program: OsString, args: Vec<CommandArg>) -> io::Result<PreparedCommandLine> {
		prepare_command_line(&program, &args)
	}
	...
	fn quotes_regular_arguments_with_windows_crt_rules() {
		...
		for (name, args, expected) in cases {
			let prepared = prepare(OsString::from("tool"), args).unwrap();
			assert_eq!(
	...
	fn preserves_lone_surrogates_in_program_and_arguments() {
		...
		let prepared = prepare(
			os(&program),
			vec![
				CommandArg::Regular(os(&[0xdc00])),
				CommandArg::Raw(os(&[0xd801])),
			],
		)
		.unwrap();
	...
	fn encodes_argv0_with_its_special_windows_rules() {
		let prepared = prepare(OsString::from(r"C:\Program Files\"), Vec::new()).unwrap();
		assert_eq!(
			prepared.command_line.as_units(),
			[
				DOUBLE_QUOTE,
	...
	fn quotes_multiple_backslash_runs_in_regular_arguments() {
		...
		let prepared = prepare(OsString::from("tool"), vec![CommandArg::Regular(arg)]).unwrap();
		assert_eq!(
			prepared.command_line.as_units(),
			[
				DOUBLE_QUOTE,
	...
	fn rejects_unrepresentable_quotes_in_argv0() {
		let error = prepare(OsString::from("bad\"program"), Vec::new()).unwrap_err();
	...
	fn rejects_embedded_nuls_with_stable_errors() {
		let cases = [
			...
			(
				OsString::from("tool"),
				vec![CommandArg::Regular(os(&[b'a' as u16, 0]))],
				"PTY argument contains an embedded NUL",
			),
			(
				OsString::from("tool"),
				vec![CommandArg::Raw(os(&[b'a' as u16, 0]))],
				"PTY raw argument contains an embedded NUL",
			),
		];

		for (program, args, message) in cases {
			let error = prepare(program, args).unwrap_err();
			assert_eq!(error.kind(), io::ErrorKind::InvalidInput);
