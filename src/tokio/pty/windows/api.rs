...
fn resolve() -> Option<ConPtyApi> {
	// SAFETY: kernel32.dll is loaded for every Windows process and GetModuleHandleW only borrows its
	// module handle.
	let kernel32 = unsafe { GetModuleHandleW(w!("kernel32.dll")) }.ok()?;
	resolve_with(|name| {
		// SAFETY: kernel32 remains loaded for the process lifetime, and name is a NUL-terminated C
		// string which remains live for the call. Returned addresses have the module's lifetime.
		unsafe { GetProcAddress(kernel32, PCSTR(name.as_ptr().cast())) }
	})
}
